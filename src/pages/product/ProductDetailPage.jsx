      ...
      {isShareOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl animate-fade-in">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-6">Compartir producto</h3>
            {images && images[0] && (
              <div className="mb-4 rounded-2xl overflow-hidden border border-gray-100">
                <img src={images[0]} alt={product.name} className="w-full h-32 object-contain" />
              </div>
            )}
            <div className="grid grid-cols-4 gap-4">
              <a
                href={`https://wa.me/?text=${encodeURIComponent('🔥 ' + product.name + '%0A%0A💰 Ahora por solo $' + product.price?.toLocaleString('es-AR') + (product.price !== product.original_price ? ' (era $' + product.original_price?.toLocaleString('es-AR') + ')' : '') + '%0A%0A' + (product.description ? product.description.substring(0, 120) + (product.description.length > 120 ? '...' : '') + '%0A%0A' : '') + (product.stock > 0 ? '✅ Disponible ahora!' : '❌ Agotado') + '%0A%0A🔗 Miralo en Kemazon: ' + window.location.href)}`}
                target="_blank"
            ...
            <div className="mt-6">
              <p className="text-sm font-bold text-gray-500 mb-2">Enlace del producto</p>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    const fullMsg = `🔥 ${product.name}\n\n💰 $${product.price?.toLocaleString('es-AR')} ${product.price !== product.original_price ? `(era $${product.original_price?.toLocaleString('es-AR')})` : ''}\n\n${product.description ? product.description.substring(0, 100) + (product.description.length > 100 ? '...' : '') + '\n' : ''}\n${product.stock > 0 ? '✅ Disponible ahora!' : '❌ Agotado'}\n\n🔗 ${window.location.href}`;
